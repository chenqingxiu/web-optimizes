...
var resizePizzas = function(size) {
  ...
  // 改变滑窗前披萨的尺寸值
  // 先读取,同时用getElementById代替querySelector
  ...
  // 通过changePizzaSizes函数可以直接实现功能，将determineDx函数作废
  ...
  function changePizzaSizes(size) {
    ...
    // 逐一改变披萨的尺寸
    // 用getElementById代替querySelector,getElementsByClassName代替querySelectorAll
...
function updatePositions() {
  ...
  // 在循环外保存document.body.scrollTop的值，减少对DOM的操作
  ...
  //将实现背景位置的函数放进requestAnimationFrame
  function updatePizza(){
    if (j == len) {
      return;
    } else {
      // 用transform实现背景中的披萨滑窗
      if(k > 4){
        k = 0;
      }
      var phase = Math.sin((scrollTop / 1250) + k);
      items[j].style.transform = "translateX("+ 100 * phase + "px)";
      j++;
      k++;
      window.requestAnimationFrame(updatePizza);
    }
  }
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // 将背景披萨的个数由i = 200改为i = 25,在不影响网页外观的同时提高性能
  for (var i = 0; i < 25; i++) {
  ...
  var len = items.length
  for (var l = 0; l < len; l++) {
    items[l].style.left = items[l].basicLeft + 'px';
  }
